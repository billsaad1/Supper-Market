...
namespace Supermarket.UI.Views
{
    public partial class DetailedReportsForm : Form
    {
        ...
        private void SetupUI()
        {
            ...
            tabs.TabPages.Add(CreateReportTab("Sales Report / تقرير المبيعات", async (dgv, from, to) => {
                ReportRepository repo = new ReportRepository(AppSettings.ConnectionString);
                var data = await repo.GetDailySalesAsync(from, to);
            ...
            tabs.TabPages.Add(CreateReportTab("Shortages / النواقص", async (dgv, from, to) => {
                ReportRepository repo = new ReportRepository(AppSettings.ConnectionString);
            ...
            tabs.TabPages.Add(CreateReportTab("Account Ledger / كشف حساب", async (dgv, from, to) => {
